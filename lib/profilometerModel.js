// frontend/lib/profilometerModel.js
// JS-порт твоего backend/app/profilometer_model.py:
// - predictRa / predictRz (регрессия + optional noise)
// - generateProfileFromRaRz (генерация профиля x,z под target Ra/Rz)
// Источник логики: profilometer_model.py :contentReference[oaicite:2]{index=2}
...
/**
 * Генерит профиль z(x) (в микрометрах), который примерно соответствует заданным Ra/Rz.
 * Возвращает объект { x: number[], z: number[] }.
 * Это порт generate_profile_from_ra_rz из Python. :contentReference[oaicite:3]{index=3}
 */
export function generateProfileFromRaRz({
  ...
  // 1) базовый “шероховатый” сигнал: белый шум -> сглаживание
  ...
  for (let s = 0; s < nSpikes; s++) {
    ...
    // форма спайка гауссом на 7 точек
    for (let d = -3; d <= 3; d++) {
      const j = idx + d;
      if (j >= 0 && j < n) {
  ...
  // 3) подстройка коэффициента спайков, чтобы приблизить Rz при сохранении Ra
  let k = 0.8;
  const eps = 1e-9;
  ...
  for (let it = 0; it < 10; it++) {
    ...
    // мягко корректируем k
    k *= Math.pow(ratio, 0.7);
  }

  // 4) добавим “измерительный шум”
  if (addNoise) {

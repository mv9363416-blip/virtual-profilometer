...
export default function Home() {
  ...
  const apOk = ap.trim() !== "" && Number.isFinite(apNum) && apNum >= 0.79 && apNum <= 21.31;
...
function drawProfileFrame(canvas, profile, t01) {
  ...
  // "головка" профилометра — позиция на текущей точке
  ...
  // каретка (прямоугольник)
  ctx.fillStyle = "rgba(0,0,0,0.18)";
  ctx.fillRect(hx - 14, railY - 10, 28, 16);

  // стилус (треугольник вниз) + линия к профилю
...
function drawOverlays(ctx, { x, zCentered, ra, highs, lows, mapX, mapY, pad, plotW, plotH, rzFromModel, showLabels }) {
  // 1) mean line (z=0)
  const y0 = mapY(0);
  ctx.strokeStyle = "rgba(0,0,0,0.18)";
  ctx.lineWidth = 1;
  ...
  // 2) Ra band: +Ra and -Ra + легкая заливка
  const yPlus = mapY(ra);
  const yMinus = mapY(-ra);

  // заливка между -Ra и +Ra
  ...
  // 4) “скобка” Rz справа: от среднего пика до среднего дна
  ...
  const rzValue = (meanHigh - meanLow);

  const yH = mapY(meanHigh);
  const yL = mapY(meanLow);

  // внутри графика справа
  const bx = pad + plotW;
...
function restartProfilometerScan() {
  ...
  if (skipAnimations) {
    ...
    if (canvasRef.current) {
      drawProfileFrame(canvasRef.current, profileRef.current, 1);
    }
    return;
  }
...
function computeMetrics(profile) {
  ...
  // сохраняем индексы, чтобы можно было рисовать точки на графике
        ...
        <Card className="p-5 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              />
              <div className="text-xs text-gray-500">{t.range} 0.79 - 21.31</div>
            </div>
            <div className="space-y-2">
